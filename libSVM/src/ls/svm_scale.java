...
public class svm_scale {
	...
	private void output_target(double value, BufferedOutputStream output_scale) {
		if (y_scaling) {
			if (value == y_min)
				value = y_lower;
			else if (value == y_max)
		...
		try {
			output_scale.write((value + " ").getBytes());
		} catch (IOException e) {
...
//		System.out.print(value + " ");
	...
	private void output(int index, double value,
			BufferedOutputStream output_scale) {
		...
		if (value != 0) {
			try {
				output_scale.write((index + ":" + value + " ").getBytes());
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
//			System.out.print(index + ":" + value + " ");
	...
	private void run(String[] argv) throws IOException {
		...
		BufferedOutputStream output_scale = null;
		...
		try {
			...
			output_scale = new BufferedOutputStream(new FileOutputStream(
					new File(argv[i] + ".scale"), false));
		} catch (Exception e) {
		...
		while (readline(fp) != null) {
			...
			StringTokenizer st = new StringTokenizer(line, " \t\n\r\f:");
			target = Double.parseDouble(st.nextToken());
			output_target(target, output_scale);
			while (st.hasMoreElements()) {
				index = Integer.parseInt(st.nextToken());
				value = Double.parseDouble(st.nextToken());
				for (i = next_index; i < index; i++)
					output(i, 0, output_scale);
